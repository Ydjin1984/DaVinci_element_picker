...
export function buildCloneMarkdown(
  ...
  lines.push(
    ...
    "",
    "1. Open **element.png** and match the visual exactly.",
...
export function buildAgentMarkdown(
  ...
  return [
    ...
    "2. Implement as clean components in this repo’s stack (do not paste huge vendor bundles unless necessary).",
    "3. Use local assets from `assets/` (see manifest). Download failures: note and approximate.",
    "4. Preserve interactive affordances visible in the capture (buttons, progress, hover styles if documented).",
    "5. If the element is a chart/canvas engine: recreate the **look** (axes, series, chrome); full engine port only if required.",
    "6. Do not invent missing content; use captured text/structure.",
    "7. When done, show a side-by-side checklist vs `element.png`.",
    "",
    "## Constraints",
    "",
    ...
    "2. Open `element.png` (and `preview.html` if present).",
